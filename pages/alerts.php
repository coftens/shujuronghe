...
<div id="tabHistory">
    <div class="flex-between mb-2">
        <div class="form-inline">
            ...
            <button class="btn" onclick="loadAlertHistory()">🔄 刷新</button>
        </div>
        <button class="btn btn-success" onclick="resolveAll()">✅ 全部解决</button>
    </div>
...
async function loadAlertHistory(page = 1) {
    ...
    if (data.list.length === 0) {
        container.innerHTML = '<div class="empty-state" style="padding:30px"><div class="empty-icon">✅</div><p>暂无告警记录</p></div>';
...
async function loadRules() {
    ...
    tbody.innerHTML = resp.data.map(r => `
        <tr>
            ...
            <td>
                <div class="btn-group">
                    <button class="btn btn-sm" onclick="editRule(${JSON.stringify(r).replace(/"/g, '&quot;')})">✏️</button>
